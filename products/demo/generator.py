"""Artifact-driven test generation for the SauceDemo product pack."""
...
def generate_tests(artifact: KnowledgeArtifact, output_directory: Path) -> Path:
    """Generate required tests after confirming their element knowledge exists."""
    ...
    known = {locator.element_id for locator in artifact.locators}
    missing = required - known
    if missing:
        raise ValueError("Artifact lacks required locators: " + ", ".join(sorted(missing)))
    output_directory.mkdir(parents=True, exist_ok=True)
    output = output_directory / "test_saucedemo_generated.py"
    output.write_text(GENERATED_TEST, encoding="utf-8")
    return output
